...
import PlantReturnModal from "./PlantReturnModal";
import {
  M4_PENDING,
  M4_HISTORY,
  fmtDate,
...
const M4_COLUMNS = [
  ...
  { key: "dnimage", label: "Debit Note Image" },
  { key: "status", label: "Status" }
];

export default function PlantReturnView({ onOpenDetails }) {
  const {
    records,
    loading,
    activeTabs,
  ...
  const isEditable = canEdit("plantReturn");
  const activeTab = activeTabs.plantReturn;

  const pendingAll = records.filter(M4_PENDING);
  const historyAll = records.filter(M4_HISTORY);

  const pendingFiltered = applyFilters(pendingAll, "plantReturn");
  const historyFiltered = applyFilters(historyAll, "plantReturn");

  const displayRows = activeTab === "pending" ? pendingFiltered : historyFiltered;
  const colVis = getColVis("m4", M4_COLUMNS);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Confirm physical loading and final gate dispatch of return materials out of plant
            </p>
          </div>

          <div className="flex items-center gap-2">
            <ColumnToggleDropdown tableKey="m4" columns={M4_COLUMNS} />
          </div>
        </div>

        <FilterBar viewKey="plantReturn" />

        <div className="flex items-center gap-6 px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <button
            type="button"
            onClick={() => setTab("plantReturn", "pending")}
          ...
          <button
            type="button"
            onClick={() => setTab("plantReturn", "history")}
            className={`py-3 text-xs font-bold border-b-2 transition-all flex items-center gap-2 ${
              activeTab === "history"
                ? "border-blue-600 text-blue-600 dark:text-blue-400"
                : "border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
        ...
        {activeTab === "pending" && (
          <BulkActionBar moduleKey="plantReturn" onTriggerBulkAction={handleBulkAction} />
        )}
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">
                ...
                {colVis.driver && <th className="py-3 px-3">Driver</th>}
                ...
                {colVis.dnimage && <th className="py-3 px-3">Debit Note Image</th>}
              ...
              {loading ? (
                ...
                groupRecordsByBill(displayRows).map((group) => {
                  const isAllGroupSelected =
                    selection.moduleKey === "plantReturn" &&
                  ...
                  return (
                    <Fragment key={group.billKey}>
                      ...
                      {group.records.map((r) => {
                        const isChecked =
                          selection.moduleKey === "plantReturn" && selection.ids.includes(r.id);
                        const status = overallStatus(r);
                        const tatStatus = getTatStatusForReturn
                          ? getTatStatusForReturn(r.id, "Return From Plant")
                          : null;
                          ...
                          >
                            ...
                            {activeTab === "pending" && (
                              <td className="py-2.5 px-3 text-center">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={() => toggleRowSelect("plantReturn", r.id, r.billNumber)}
                                  className="rounded border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-blue-500 h-3.5 w-3.5 cursor-pointer"
                                />
                              </td>
                            )}
                              ...
                              >
                                {tatStatus?.dueAt ? (
                                  <TatStageBadge
                                    tatStatus={tatStatus}
                                    isCompleted={
                                      activeTab === "history" || Boolean(r.dispatch)
                                    }
                                  />
                                ) : (
                            ...
                            {colVis.driver && (
                              <td className="py-2.5 px-3 text-slate-600 dark:text-slate-400 whitespace-nowrap">
                                {r.logistics ? r.logistics.driverName : "-"}
                              </td>
                            )}
                            {colVis.bilty && (
                              <td className="py-2.5 px-3 font-mono text-slate-600 dark:text-slate-400 whitespace-nowrap">
                            ...
                            {colVis.dn && (
                              <td className="py-2.5 px-3 font-mono text-slate-700 dark:text-slate-300 whitespace-nowrap">
                                {r.debitNote ? r.debitNote.number : <span className="text-slate-400 italic">N/A</span>}
                              </td>
                            )}
                            {colVis.dnamt && (
                            ...
                            {colVis.dnimage && (
                              <td className="py-2.5 px-3 whitespace-nowrap">
                                {r.debitNote && r.debitNote.imageName ? (
                                  <a
                                    href={
                                      r.debitNote.imageUrl ||
                                      placeholderPreviewUrl(r.debitNote.imageName)
                                    }
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1 text-[11px] font-mono font-semibold text-blue-600 hover:underline"
                                  >
