...
const M3_COLUMNS = [
  ...
  { key: "creditnoteimage", label: "Credit Note Image" },
...
export default function DebitNoteView({ onOpenDetails }) {
  const {
    records,
    loading,
    activeTabs,
  ...
  const isEditable = canEdit("debitNote");
  const activeTab = activeTabs.debitNote;

  const pendingAll = records.filter(M3_PENDING);
  const historyAll = records.filter(M3_HISTORY);

  const pendingFiltered = applyFilters(pendingAll, "debitNote");
  const historyFiltered = applyFilters(historyAll, "debitNote");

  const displayRows = activeTab === "pending" ? pendingFiltered : historyFiltered;
  const colVis = getColVis("m3", M3_COLUMNS);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
              Issue Debit Note &amp; Inform
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Generate debit notes against approved returns and officially notify suppliers
            </p>
          </div>

          <div className="flex items-center gap-2">
            <ColumnToggleDropdown tableKey="m3" columns={M3_COLUMNS} />
          </div>
        </div>

        <FilterBar viewKey="debitNote" />

        <div className="flex items-center gap-6 px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <button
            type="button"
            onClick={() => setTab("debitNote", "pending")}
          ...
          <button
            type="button"
            onClick={() => setTab("debitNote", "history")}
            className={`py-3 text-xs font-bold border-b-2 transition-all flex items-center gap-2 ${
              activeTab === "history"
                ? "border-blue-600 text-blue-600 dark:text-blue-400"
                : "border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300"
        ...
        {activeTab === "pending" && (
          <BulkActionBar moduleKey="debitNote" onTriggerBulkAction={handleBulkAction} />
        )}
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 text-[10px] uppercase font-bold tracking-wider">
                ...
                {colVis.creditnoteimage && <th className="py-3 px-3">Credit Note Image</th>}
                ...
                groupRecordsByBill(displayRows).map((group) => {
                  ...
                  return (
                    <Fragment key={group.billKey}>
                      ...
                      {group.records.map((r) => {
                        const isChecked =
                          selection.moduleKey === "debitNote" && selection.ids.includes(r.id);
                        const status = overallStatus(r);
                        const tatStatus = getTatStatusForReturn
                          ? getTatStatusForReturn(r.id, "Issue Debit Note")
                          : null;
                        ...
                        return (
                          ...
                          >
                            ...
                            {activeTab === "pending" && (
                              <td className="py-2.5 px-3 text-center">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={() => toggleRowSelect("debitNote", r.id, r.billNumber)}
                                  className="rounded border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-blue-500 h-3.5 w-3.5 cursor-pointer"
                                />
                              </td>
                            )}
                                ...
                                {tatStatus?.dueAt ? (
                                  <TatStageBadge
                                    ...
                                    isCompleted={
                                      ...
                                      Boolean(
                                        r.debitNote ||
                                          (r.debitNotes && r.debitNotes.length > 0)
                                      )
                                    }
                                  />
                                ) : (
                            ...
                            {colVis.creditnoteimage && (
                              <td className="py-2.5 px-3 whitespace-nowrap">
                                {r.creditNote && r.creditNote.attachmentName ? (
                                  <a
                                    href={
                                      r.creditNote.attachmentUrl ||
                                      placeholderPreviewUrl(r.creditNote.attachmentName)
                                    }
                                    target="_blank"
                                    rel="noreferrer"
