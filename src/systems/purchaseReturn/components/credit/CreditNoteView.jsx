...
export default function CreditNoteView({ onOpenDetails }) {
  const {
    records,
    loading,
    activeTabs,
  ...
  const activeTab = activeTabs.credit;

  const pendingAll = records.filter(MC_PENDING);
  const historyAll = records.filter(MC_HISTORY);

  const pendingFiltered = applyFilters(pendingAll, "credit");
  const historyFiltered = applyFilters(historyAll, "credit");

  const displayRows = activeTab === "pending" ? pendingFiltered : historyFiltered;
  const colVis = getColVis("mc", MC_COLUMNS);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden transition-colors">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            <h3 className="text-sm font-bold text-slate-900 dark:text-slate-100">
              Ask Party For Credit Note
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Request supplier or vendor to issue credit note before dispatch or debit note issuance
            </p>
          </div>

          <div className="flex items-center gap-2">
            <ColumnToggleDropdown tableKey="mc" columns={MC_COLUMNS} />
          </div>
        </div>

        <FilterBar viewKey="credit" />

        <div className="flex items-center gap-6 px-4 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <button
            type="button"
            onClick={() => setTab("credit", "pending")}
        ...
        {activeTab === "pending" && (
          <BulkActionBar moduleKey="credit" onTriggerBulkAction={handleBulkAction} />
        )}
              ...
              {loading ? (
                ...
                groupRecordsByBill(displayRows).map((group) => {
                  ...
                  const groupTotalVal = group.records.reduce(
                    (s, r) => s + (Number(totalReturnValue(r)) || 0),
                    0
                  );
                  const totalColCount =
                    1 +
                            ...
                            {activeTab === "pending" && (
                              <td className="py-2.5 px-3 text-center">
                                <input
                                  type="checkbox"
                                  checked={isChecked}
                                  onChange={() => toggleRowSelect("credit", r.id, r.billNumber)}
                                  className="rounded border-slate-300 dark:border-slate-600 text-blue-600 focus:ring-blue-500 h-3.5 w-3.5 cursor-pointer"
                                />
                              </td>
                            )}
