      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl shadow-xs p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
            ...
            >
              <span>Pending Decision {loading || isRefreshing || (isBackgroundStreaming && pendingList.length === 0) ? "(...)" : `(${pendingList.length})`}</span>
            </button>
            <button
              type="button"
              onClick={() => {
            ...
            >
              <span>Sanctioned History {loading || isRefreshing || (isBackgroundStreaming && historyList.length === 0) ? "(...)" : `(${historyList.length})`}</span>
            </button>
          </div>
        </div>
                ...
                <tr>
                  ...
                  >
                    <CircularProcessingLoader
                      message={`Loading & processing ${activeTab === "pending" ? "pending vendor decisions" : "completed vendor approvals"}...`}
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-2xl w-full overflow-hidden flex flex-col">
            ...
            >
              ...
              {/* Row 2: Payment Terms & Expected Delivery (Editable) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold uppercase tracking-wide text-slate-600 dark:text-slate-400 block">
                    Payment Terms
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. 30 days, 50% Advance, Immediate..."
                    ...
                    className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-semibold text-slate-800 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-mono font-semibold text-slate-800 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-blue-500 cursor-pointer"
                  />
                </div>
              </div>
