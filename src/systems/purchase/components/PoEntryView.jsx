...
import { formatDateDash, formatDateTime, formatForDateInput, toLocalIsoTimestamp } from "../utils/dateUtils";

export default function PoEntryView() {
  const { showToast } = useMagicToast();
  const { indents, purchaseOrders, approvedVendors, quotations, createPurchaseOrder, revisePurchaseOrder, refreshData, getIndentNumber } = usePurchaseWorkflow();
  ...
  const [billingAddress, setBillingAddress] = useState(ADDRESS_OPTIONS[0].address);
  ...
  const combinedAddressOptions = useMemo(() => {
    const list = dbAddresses && dbAddresses.length > 0 ? dbAddresses : ADDRESS_OPTIONS;
    return list.map((a) => {
  ...
  useEffect(() => {
    const loadMasters = async () => {
      try {
        const [vList, whList, addrList, ttList, gstList, poTermsList, systemLookups] = await Promise.all([
          fetchMasterVendors(),
          fetchMasterWarehouses(),
          fetchMasterAddresses(),
          fetchMasterTransportTypes(),
        ...
        if (systemLookups?.items || systemLookups?.rawMaterials || systemLookups?.finishedGoods || systemLookups?.materials) {
          const combined = [
    ...
    return indents
      ...
      .map((r) => {
        ...
        return {
          ...
          approverName: r.approver_name || "Approver",
          vendorName: vendorName,
          rate: rate > 0 ? `₹${rate.toLocaleString()}` : "To Fill",
          totalAmount: total > 0 ? `₹${total.toLocaleString(undefined, { maximumFractionDigits: 0 })}` : "—",
          freightType: r.transport_type || av?.transport_type || "Ex-Factory + Transport",
          paymentTerms: r.payment_terms || av?.payment_terms || "30",
          expDelivery: r.lead_time || r.delivery_terms || r.delivery_date || null,
        };
      })
  ...
  const historyList = useMemo(() => {
    return purchaseOrders
      ...
      .filter((po) => divisionFilter === "all" || po.delivery_location === divisionFilter)
      .filter((po) => {
        const s = searchTerm.toLowerCase();
        if (!s) return true;
        return (
  ...
  const warehouseOptions = useMemo(() => {
    ...
    return list.map((w) => (typeof w === "string" ? w : w.name || w.warehouse_name));
  }, [dbWarehouses]);
  ...
  const updateSupplierFields = (vName) => {
    ...
    if (foundVendor) {
      ...
      setSupplierContactPerson(foundVendor.contact_person || foundVendor.contactPerson || "");
  ...
  const handleOpenCreatePO = (targetRow = null) => {
    ...
    if (items.length === 0) {
      if (showToast) showToast("Please select at least one pending requisition", "warning");
      return;
    }
    ...
    const primaryAv = (approvedVendors || []).find((a) => a.indent_id === primary.id) || primary.approved_vendor;
    const primaryQuotes = primary.quotation_submissions || (quotations || []).filter((q) => q.indent_id === primary.id) || [];
    const primaryQuote =
      primaryQuotes.find((q) => (primaryAv?.selected_quotation_id && q.id === primaryAv.selected_quotation_id) || q.is_selected) ||
      primaryQuotes.find((q) => q.vendor_name === (primary.selected_vendor_name || primaryAv?.vendor_name)) ||
      primaryQuotes[0];
    ...
    const rawDivision = primary.warehouse_location || primary.division_name || "";
    const matchedAddress = (dbAddresses || []).find((a) => {
      const aDiv = a.division || a.division_name || "";
      const aName = a.name || "";
      return (
        (aDiv && rawDivision && aDiv.toLowerCase() === rawDivision.toLowerCase()) ||
        (aName && rawDivision && aName.toLowerCase().includes(rawDivision.toLowerCase()))
      );
    });
    ...
    const matchedAddrObj = (dbAddresses || []).find((a) => cleanCompanyName(a.name) === defaultAddressName);
    if (matchedAddrObj) {
      setDestAddress(matchedAddrObj.address || matchedAddrObj.address_line || NUTECH_ADDRESS);
    }
    ...
    const lines = {};
    items.forEach((it) => {
      const av = (approvedVendors || []).find((a) => a.indent_id === it.id) || it.approved_vendor;
      const quotes = it.quotation_submissions || (quotations || []).filter((q) => q.indent_id === it.id) || [];
      const selectedQuote =
        quotes.find((q) => (av?.selected_quotation_id && q.id === av.selected_quotation_id) || q.is_selected) ||
        quotes.find((q) => q.vendor_name === (it.selected_vendor_name || av?.vendor_name)) ||
        quotes[0];
      ...
      const matchedMat = (catalogMaterials || []).find(
        (m) =>
          (m.name && itemName && m.name.toLowerCase().trim() === itemName.toLowerCase()) ||
          (m.item_name && itemName && m.item_name.toLowerCase().trim() === itemName.toLowerCase()) ||
          (m.sku && itemCode && m.sku.toLowerCase().trim() === itemCode.toLowerCase()) ||
          (m.item_code && itemCode && m.item_code.toLowerCase().trim() === itemCode.toLowerCase())
      );
      const detectedHsn = matchedMat?.hsn_code || matchedMat?.hsnCode || matchedMat?.hsn || it.hsn_code || it.hsnCode || (itemCode && /^\d{4,8}$/.test(itemCode) ? itemCode : "") || "7216";
      const detectedGst = selectedQuote?.gst_percent != null
  ...
  const handleOpenRevisePO = (po) => {
    ...
    const defaultReviseAddr = cleanCompanyName(po.delivery_location || (dbAddresses && dbAddresses[0] ? dbAddresses[0].name : ADDRESS_OPTIONS[0].name));
    setFirmName(po.firm_name || "Nutech Pipes");
    setDeliveryLocation(defaultReviseAddr);
    setDestName(defaultReviseAddr);
    const matchedAddrObj = (dbAddresses || []).find((a) => cleanCompanyName(a.name) === defaultReviseAddr);
    if (matchedAddrObj) {
      setDestAddress(matchedAddrObj.address || matchedAddrObj.address_line || NUTECH_ADDRESS);
    }
    setTransportType(po.transport_type || "F.O.R.");
    setPaymentTerms(po.payment_type || "30");
    updateSupplierFields(po.vendor_name || "");
    setQuotationNumber(po.quotation_number || `QUO-${po.po_number}`);
    setQuotationDate(po.quotation_date || po.po_date || new Date().toISOString().split("T")[0]);
    setAdvancePayment(Number(po.advance_amount) > 0 ? "yes" : "no");
    setAdvanceAmount(String(po.advance_amount || 0));
    setRemarks(po.remarks || "");
    setTerms(po.terms || po.termsList || DEFAULT_TERMS);
    ...
    const mockItem = {
      ...
      uom: po.uom,
      indent_number: po.indent_id || po.po_number,
    };
    setSelectedIndents([mockItem]);

    const matchedMat = (catalogMaterials || []).find(
      (m) =>
        (m.name && po.item_name && m.name.toLowerCase().trim() === String(po.item_name).toLowerCase().trim()) ||
        (m.item_name && po.item_name && m.item_name.toLowerCase().trim() === String(po.item_name).toLowerCase().trim()) ||
        (m.sku && po.item_code && m.sku.toLowerCase().trim() === String(po.item_code).toLowerCase().trim()) ||
        (m.item_code && po.item_code && m.item_code.toLowerCase().trim() === String(po.item_code).toLowerCase().trim())
    );
    const resolvedHsn = po.hsn_code || po.hsn || matchedMat?.hsn_code || matchedMat?.hsnCode || "7216";

    const lines = {
      [mockItem.id]: {
        rate: String(po.unit_rate || 0),
        hsn: String(resolvedHsn),
        gst: String(po.gst_rate || "18").replace("%", ""),
        deliveryDate: "",
      },
    };
    setLineItems(lines);
    ...
    try {
      if (poMode === "create") {
        for (const item of selectedIndents) {
          ...
          const finalVendorName = supplierName.trim() || item.vendorName || item.selected_vendor_name || (dbVendors[0]?.vendor_name || dbVendors[0]?.name || "Approved Supplier");
          ...
          await createPurchaseOrder({
            ...
            advance_amount: advancePayment === "yes" ? Number(advanceAmount || 0) : 0,
            advanceAmount: advancePayment === "yes" ? Number(advanceAmount || 0) : 0,
            payment_type: paymentTerms === "Custom" ? customPaymentTerms : paymentTerms,
            paymentTerms: paymentTerms === "Custom" ? customPaymentTerms : paymentTerms,
            transport_type: transportType !== "Select Transport Type" ? transportType : "F.O.R.",
            transportType: transportType !== "Select Transport Type" ? transportType : "F.O.R.",
            delivery_location: deliveryLocation,
            deliveryLocation,
            delivery_date: quotationDate ? new Date(quotationDate).toISOString() : null,
            po_date: poDate ? new Date(poDate).toISOString() : new Date().toISOString(),
            poDate: poDate ? new Date(poDate).toISOString() : new Date().toISOString(),
            remarks,
            terms: terms || [],
            termsList: terms || [],
          });
        }
        if (showToast) showToast(`Purchase Order ${poNumber} generated successfully!`, "success");
      } else {
      ...
      if (showToast) showToast("Failed to save Purchase Order: " + err.message, "error");
    } finally {
  ...
  const handleDownloadIssuedPoPdf = (po) => {
    try {
      ...
      const readableIndent = po.indent_number || (getIndentNumber ? getIndentNumber(po.indent_id) : null) || (po.indent_id && po.indent_id.length > 8 ? `IND-${po.indent_id.slice(0, 8).toUpperCase()}` : "IND-2026-001");
      const locName = po.delivery_location || po.deliveryLocation || "M/S Nutech Pvt. Ltd.";

      // Match destination address from master_addresses / ADDRESS_OPTIONS
      const matchedAddr = (dbAddresses || []).find(
        (a) => (a.name || "").toLowerCase() === locName.toLowerCase() ||
               cleanCompanyName(a.name || "").toLowerCase() === cleanCompanyName(locName).toLowerCase()
      ) || ADDRESS_OPTIONS.find(
        (a) => (a.name || "").toLowerCase() === locName.toLowerCase() ||
               cleanCompanyName(a.name || "").toLowerCase() === cleanCompanyName(locName).toLowerCase()
      );
      const finalDestAddress = matchedAddr?.address || po.destinationAddress || po.delivery_location_address || NUTECH_ADDRESS;

      generatePoPdf({
        poNumber: po.po_number || "PO-2026-001",
        poDate: po.po_date || po.actualDate || po.timestamp || new Date().toISOString().split("T")[0],
        vendorName: po.vendor_name || "Supplier",
        vendorAddress: po.vendor_address || `${po.vendor_name || "Supplier"} Industrial Complex`,
        vendorContact: po.vendor_contact || "Authorized Representative",
        vendorPhone: po.vendor_phone || "9123456789",
        vendorEmail: po.vendor_email || `sales@${(po.vendor_name || "vendor").toLowerCase().replace(/\s+/g, "")}.com`,
        vendorGstin: po.vendor_gstin || "22AAAPL1234A1Z5",
        
        // Billing & Destination
        consigneeName: po.consigneeName || po.firm_name || "Nutech Pipes Pvt. Ltd.",
        consigneeAddress: po.consigneeAddress || NUTECH_ADDRESS,
        billingName: po.consigneeName || po.firm_name || "Nutech Pipes Pvt. Ltd.",
        ...
        // Order References
        expectedDeliveryDate: po.delivery_date || po.expected_delivery_date || po.delivery_terms || "7 to 10 days",
        quotationNumber: po.quotation_number || po.quotation_no || "-",
        quotationDate: po.quotation_date || "-",
        paymentTerms: po.payment_type ? `Advance Payment (${po.advance_percentage || 0}%)` : (po.payment_terms || "30 Days Credit"),
        ...
        items: po.items && po.items.length > 0 ? po.items.map(it => ({
          ...it,
          indentNumber: it.indent_number || (getIndentNumber ? getIndentNumber(it.indent_id || po.indent_id) : null) || readableIndent,
        })) : [
          {
            srNo: 1,
            ...
            gstPercent: gstVal,
            amount: po.total_amount || (rateVal * qtyVal * (1 + Number(gstVal) / 100)),
          },
        ],
        totalAmount: po.total_amount,
        termsList: po.termsList || po.terms || [],
      });
      if (showToast) showToast(`Opening PO ${po.po_number || "Draft"} PDF in a new tab...`, "info");
    } catch (e) {
      console.error("PDF Download error:", e);
      if (showToast) showToast(`Failed to generate PO PDF: ${e.message}`, "error");
    }
  };
  ...
  const handleDownloadDraftPdf = () => {
    try {
      ...
      generatePoPdf({
        ...
        destinationAddress: destAddress || NUTECH_ADDRESS,
        deliveryLocation: deliveryLocation || destName || "M/S Nutech Pvt. Ltd.",
        expectedDeliveryDate: Object.values(lineItems).find((l) => l.deliveryDate)?.deliveryDate || "7 to 10 days",
        quotationNumber: quotationNumber || "-",
        quotationDate: quotationDate || "-",
        paymentTerms: paymentTerms === "Custom" ? customPaymentTerms : (advancePayment === "yes" ? `Advance Payment (${advanceAmount}%)` : paymentTerms),
        advanceAmount: advancePayment === "yes" ? parseFloat(advanceAmount) || 0 : 0,
      ...
      if (showToast) showToast("Failed to preview PO PDF: " + e.message, "error");
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Generate official purchase orders for sanctioned suppliers and manage revised commercial contracts.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            ...
            >
              <option key="div-all" value="all">All Divisions</option>
          ...
          <table className="w-full text-left text-xs border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={12} className="p-8 text-center text-slate-400">
                    No {activeTab === "pending" ? "pending PO items" : "issued PO records"} found.
                  </td>
                </tr>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3 text-center font-mono font-semibold text-slate-700 dark:text-slate-300">
                          {formatDateDash(row.expDelivery)}
                        </td>
                      </tr>
                    );
                  } else {
                    /* History Row (Exact 12 Columns with Rich Formatting from po-entry.tsx) */
                    const basicVal = Number(row.basic_value || (Number(row.quantity || 1) * Number(row.unit_rate || 0)));
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors">
                        ...
                        <td className="p-3">
                          <div className="space-y-0.5">
                            <div className="font-bold text-blue-900 dark:text-blue-400 font-mono text-xs">
                              {row.indentNumber || row.indent_number || (getIndentNumber ? getIndentNumber(row.indent_id) : row.indent_id) || "IND-2026-001"}
                            </div>
                            <div className="text-xs font-semibold text-slate-800 dark:text-slate-100 truncate max-w-[200px]" title={row.item_name}>
                              {row.item_name}
                            </div>
                        ...
                        <td className="p-3 text-center font-mono text-emerald-600 dark:text-emerald-400 font-semibold text-xs">
                          {formatDateTime(row.actualDate || row.timestamp || row.created_at || row.po_date)}
                        </td>
                        ...
                        <td className="p-3">
                          <div className="space-y-0.5">
                            ...
                            <div className="text-[11px] text-slate-500 font-mono">
                              Rate: ₹{Number(row.unit_rate || 0).toLocaleString()}
                            </div>
                          </div>
                        </td>
                          ...
                          <div className="space-y-0.5 text-xs">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-slate-800 dark:text-slate-200 font-medium">
                                {row.payment_type ? `Advance Payment (₹${advVal.toLocaleString()})` : "30 Days Credit"}
                              </span>
                            </div>
                            <div className="flex items-center gap-1">
                        ...
                        <td className="p-3">
                          <div className="space-y-0.5 text-xs min-w-[150px]">
                            <div className="flex justify-between items-center gap-2">
                              <span className="text-[11px] text-slate-400">Basic:</span>
                            ...
                            {advVal > 0 && (
                              <div className="flex justify-between items-center gap-2">
                                <span className="text-[11px] text-indigo-600 font-semibold">Advance:</span>
                        ...
                        {/* 11. Remarks */}
                        <td className="p-3 text-slate-600 dark:text-slate-400 italic text-xs max-w-[200px] truncate" title={row.remarks}>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in overflow-y-auto">
          <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-6xl w-full my-4 overflow-hidden flex flex-col max-h-[94vh]">
            ...
            <form onSubmit={handleSubmitPO} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
              {/* TOP BANNER CARD */}
              <section className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-6 bg-slate-50/70 dark:bg-slate-800/40 px-6 py-5 text-center sm:text-left">
                  ...
                  <div className="max-w-lg">
                    <h2 className="text-lg font-bold text-slate-900 dark:text-white">Nutech</h2>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      {dbTransportTypes.map((t, idx) => (
                        <option key={`tt-${t.id || t.name}-${idx}`} value={t.name}>
                          {t.name}
                        </option>
                      ))}
                    </select>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                  ...
                  <div className="space-y-2.5 p-4 text-xs">
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <span className="font-mono font-bold text-slate-800 dark:text-slate-200">27ABCDE1234A1Z5</span>
                    </div>
                    <div className="flex justify-between items-center gap-2">
                      <span className="font-semibold text-slate-500">PAN</span>
                      <span className="font-mono font-bold text-slate-800 dark:text-slate-200">ABCDE1234A</span>
                    </div>
                  </div>
                </section>
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                  ...
                  <div className="p-4 space-y-2 text-xs">
                    <select
                      ...
                      onChange={(e) => {
                        const val = e.target.value;
                        const opt = combinedAddressOptions.find((a) => a.name === val);
                        setBillingName(val);
                    ...
                    >
                      {combinedAddressOptions.map((a, idx) => (
                        <option key={`bill-addr-${a.name}-${idx}`} value={a.name}>
                          {a.name}
                        </option>
                      ))}
                    </select>
                    <p className="text-slate-600 dark:text-slate-400 line-clamp-2 text-[11px]" title={billingAddress}>
                      {billingAddress}
                    </p>
                  </div>
                </section>
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                  ...
                  <div className="p-4 space-y-2 text-xs">
                    ...
                    >
                      {combinedAddressOptions.map((a, idx) => (
                        <option key={`dest-addr-${a.name}-${idx}`} value={a.name}>
                          {a.name}
                        </option>
                      ))}
                    </select>
                    <p className="text-slate-600 dark:text-slate-400 line-clamp-2 text-[11px]" title={destAddress}>
                      {destAddress}
                    </p>
                  </div>
                </section>
                              ...
                              <div className="inline-flex items-center gap-1">
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setLineItems({
                                      ...lineItems,
                                      [item.id]: { ...line, rate: e.target.value },
                                    })
                                  }
                                  placeholder="0.00"
                            ...
                            <td className="p-3 text-center">
                              ...
                              >
                                {dbGstRates.map((g, gIdx) => (
                                  <option key={`gst-opt-${g.value}-${gIdx}`} value={g.value}>
                                    {g.label}
                                  </option>
                                ))}
                              </select>
                            ...
                            <td className="p-3 text-center">
                              <input
                                ...
                                onChange={(e) =>
                                  setLineItems({
                                    ...lineItems,
                                    [item.id]: { ...line, deliveryDate: e.target.value },
                                  })
                                }
                ...
                <div className="flex justify-end border-t border-slate-100 dark:border-slate-800 p-4">
                  <div className="w-64 space-y-2 text-xs">
                    <div className="flex justify-between text-slate-600 dark:text-slate-400">
                      <span>Subtotal</span>
                      <span className="font-mono font-medium">Rs. {calculatedTotals.subTotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-slate-600 dark:text-slate-400">
                      <span>GST</span>
                      <span className="font-mono font-medium">Rs. {calculatedTotals.totalTax.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t border-slate-200 dark:border-slate-700 text-sm font-bold">
                      <span className="text-slate-900 dark:text-white uppercase tracking-wider">GRAND TOTAL</span>
                ...
                <div className="divide-y divide-slate-100 dark:divide-slate-800 p-2">
                  {terms.length === 0 ? (
                    <div className="p-4 text-center text-slate-400 text-xs">
                      No terms added yet. Type a term above to add it to this Purchase Order.
                    </div>
                  ) : (
                    terms.map((term, index) => (
                      <div key={index} className="grid grid-cols-[2rem_1fr_2rem] items-center gap-2 px-3 py-2 text-xs">
              ...
              <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 -mx-6 -mb-6 p-4 px-6">
                <button
                  ...
                  onClick={() => {
                    const primary = selectedIndents[0];
                    if (primary) updateSupplierFields(primary.vendorName || primary.selected_vendor_name || (dbVendors[0]?.vendor_name || dbVendors[0]?.name || ""));
                  }}
