...
export default function AccountDataPage({
  ...
  const handleSubmit = async () => {
    ...
    if (missingStatus.length > 0)
      return alert("Please select Status (Yes/No) for all selected items.");
    ...
    try {
      const submissionData = selectedIds.map((id) => {
        ...
        return {
          ...
          remarks:
            remarksData[id] ||
            (additionalData[id] === "no" ? "Not completed" : ""),
          image: img
            ? {
                name: img.file.name,
                type: img.file.type,
  ...
  const handleMarkMultipleDone = async () => {
    ...
    if (!confirm(`Mark ${selectedHistoryItems.length} items as Admin Done?`))
      return;
  ...
  const content = (
    <div className="space-y-4 h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="flex flex-col gap-3 flex-shrink-0">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 bg-gradient-to-r from-blue-50 to-pink-50 p-3 border border-blue-100 rounded-lg shadow-sm">
          <div className="relative flex-grow w-full sm:w-auto">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-400 w-4 h-4" />
            <input
              className="w-full pl-9 pr-3 py-1.5 text-sm border border-blue-200 rounded-md focus:ring-1 focus:ring-blue-400 outline-none bg-white/80"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto flex-shrink-0">
            <button
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-1.5 text-sm font-medium text-blue-700 bg-white border border-blue-200 rounded-md hover:bg-blue-50 transition-colors shadow-sm"
            >
              {showHistory ? (
            ...
            {!showHistory && (
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50 shadow-sm transition-colors"
              >
                {isSubmitting ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50 sticky top-0 z-20 transition-all">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap bg-yellow-50">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {filteredData.map((item, idx) => {
                ...
                const isHistorySelected = selectedHistoryItems.includes(
                  item.id,
                );
                ...
                return (
                  <tr
                    key={item.id || idx}
                    className={`hover:bg-gray-50 transition-colors ${isSelected || isHistorySelected ? "bg-blue-50" : ""}`}
                  >
                      ...
                      {!showHistory ? (
                        ...
                        userRole === "admin" && (
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked)
                                setSelectedHistoryItems((p) => [...p, item.id]);
                              else
                                setSelectedHistoryItems((p) =>
                    ...
                    <td className="px-3 py-4 text-sm text-gray-600 whitespace-nowrap align-top bg-yellow-50">
                      {item.task_start_date
                        ? new Date(item.task_start_date).toLocaleString(
                            "en-IN",
                            {
                              day: "2-digit",
                              month: "2-digit",
                    ...
                    <td className="px-3 py-4 text-sm text-gray-600 whitespace-nowrap align-top">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs uppercase font-bold border ${
                    ...
                    <td className="px-3 py-4 align-top">
                      {showHistory ? (
                        <span
                          className={`inline-flex px-2 py-0.5 text-xs font-bold uppercase rounded-full border 
                        ...
                        <select
                          className={`w-full text-sm border rounded p-1.5 outline-none focus:border-gray-400 cursor-pointer
                            ${
                              additionalData[item.id] === "yes"
                                ? "bg-green-50 border-green-300 text-green-700"
                                : additionalData[item.id] === "no"
                    ...
                    <td className="px-3 py-4 align-top">
                      {showHistory ? (
                        ...
                        <textarea
                          rows={1}
                          className="w-full text-sm border border-gray-300 rounded p-1.5 focus:border-blue-500 outline-none resize-none overflow-hidden focus:ring-1 focus:ring-blue-200 transition-shadow"
                      ...
                      {showHistory ? (
                        item.image ? (
                          <a
                            href={item.image}
                            target="_blank"
                            className="text-blue-600 hover:text-blue-800 hover:underline text-sm flex items-center gap-1 font-medium"
                          >
                        ...
                        <div className="flex items-center gap-2">
                          <label
                            className={`
                              ...
                              ${uploadedImages[item.id] ? "bg-green-50 border-green-300 text-green-700 shadow-sm" : "border-gray-300 text-gray-500 hover:bg-gray-50 hover:border-blue-300 hover:text-blue-600"}
                           `}
                          >
