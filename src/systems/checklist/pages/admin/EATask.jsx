...
function TaskCard({
  ...
  const getFilteredDoers = (searchValue = "") => {
    ...
    return allDoers.filter((d) => {
      ...
      // Leave filter
      if (
        (d.status === "on leave" || d.status === "on_leave") &&
        d.leave_date &&
        d.leave_end_date
      ) {
      ...
      <div className="p-5 space-y-4">
        ...
        <div>
          <label className="block text-xs font-bold text-gray-600 mb-1.5 uppercase tracking-wide">
            Assign From (Given By) <span className="text-red-500">*</span>
          </label>
          <select
            name="given_by"
            value={task.given_by}
            ...
            <input
              ...
              onBlur={() => {
                if (
                  ...
                  return;
                setTimeout(
                  () => onUpdate(task.id, { showSuggestions: false }),
                  200,
                );
              }}
              placeholder="Enter or select doer name"
              className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-gray-50 focus:bg-white transition-all text-sm read-only:opacity-75 read-only:cursor-not-allowed"
              autoComplete="off"
            />
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="relative">
            ...
            <button
              ...
              className={`w-full px-3 py-2.5 text-left border border-gray-200 rounded-lg bg-gray-50 hover:bg-white focus:ring-2 focus:ring-blue-500 transition-all flex items-center justify-between text-sm ${task.dateLocked ? "opacity-70 cursor-not-allowed" : ""}`}
              disabled={task.dateLocked}
            >
              <span
                className={
                  task.planned_date ? "text-gray-800" : "text-gray-400"
                }
              >
                {task.planned_date
                  ? formatDateLong(new Date(task.planned_date + "T00:00:00"))
                  : "Select date"}
              </span>
            ...
            {task.showCalendar && (
              <div className="absolute top-full left-0 mt-1 z-50">
                <CalendarComponent
                  date={
                    task.planned_date
                      ? new Date(task.planned_date + "T00:00:00")
                      : null
                  }
                  onChange={(date) =>
                    onUpdate(task.id, {
                      planned_date: formatDateISO(date),
                      showCalendar: false,
                    })
                  }
          ...
          <div>
            ...
            <input
              ...
              onChange={handleInputChange}
              className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm"
            />
          </div>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl border text-xs font-bold transition-all duration-200 ${task.attachment ? "bg-blue-50 border-blue-200 text-blue-700 shadow-sm" : "bg-gray-50 border-gray-200 text-gray-400 group-hover:border-gray-300"}`}
          >
            <div className="flex items-center gap-2">
              <Plus
                className={`w-3.5 h-3.5 transition-transform ${task.attachment ? "rotate-45" : ""}`}
              />
              <span>Require Attachment / Proof</span>
            </div>
            <div
              className={`w-9 h-5 flex items-center rounded-full p-1 transition-colors ${task.attachment ? "bg-blue-600" : "bg-gray-300"}`}
            >
              <div
                className={`bg-white w-3 h-3 rounded-full shadow-sm transform transition-transform ${task.attachment ? "translate-x-4" : ""}`}
              />
            </div>
          </button>
          {task.attachment && (
            <p className="mt-1.5 px-1 text-[10px] text-blue-500 font-medium">
...
export default function EATask() {
    ...
    if (userData && userData.length > 0 && historicalDoers.length > 0) {
      ...
      if (latest && !latest.doer_name) {
        ...
        const matched = userData.find(
          (u) => (u.user_name || "").toLowerCase() === currentU,
        );
        if (matched) {
          setTasks((prev) =>
            prev.map((t, idx) =>
              idx === prev.length - 1
                ? {
                    ...t,
                    doer_name: matched.user_name,
                    phone_number: matched.phone || matched.number || "",
                  }
                : t,
            ),
          );
  ...
  const fetchUniqueDoers = async () => {
    try {
      ...
      setHistoricalDoers(
        Object.keys(doersMap).map((name) => ({ name, phone: doersMap[name] })),
      );
    } catch (err) {
    ...
    setTasks((prev) => {
      ...
      return [
        ...prev,
        {
          ...
          phone_number:
            lastTask?.phone_number ||
            (role === "user" ? localStorage.getItem("phone") || "" : ""),
  ...
  const handleSubmitAll = async () => {
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (!t.given_by) {
        showToast(
          `Task ${i + 1}: Please specify 'Assign From' (Given By).`,
          "error",
        );
        return;
      }
      if (
        !t.doer_name ||
        !t.planned_date ||
        (!t.task_description && !t.recordedAudio)
      ) {
        showToast(
          `Task ${i + 1}: Please fill in all required fields.`,
          "error",
        );
        return;
      }
      ...
      if (isH) {
        showToast(
          `Task ${i + 1}: The selected date (${dateStr}) is a holiday. Please select a different day.`,
          "error",
        );
        return;
      }
    ...
    try {
      ...
      const tasksToInsert = tasks.map((task) => {
        const startDate = new Date(
          `${task.planned_date}T${task.planned_time || "00:00"}:00`,
        );
        return {
        ...
        if (insertedData && insertedData.length > 0) {
          for (const task of insertedData) {
            await sendTaskAssignmentNotification({
              ...
              startDate: new Date(task.planned_date).toLocaleString("en-IN", {
                dateStyle: "medium",
                timeStyle: "short",
              }),
      ...
      showToast(
        `${tasksToInsert.length} EA Task(s) assigned successfully!`,
        "success",
      );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                Create and manage executive assistant tasks
              </p>
            </div>
          </div>
          <button
        ...
        <div className="mt-5 bg-white rounded-2xl border border-gray-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                All tasks will be submitted at once
              </p>
            </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-2 flex-grow py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md transform transition-all active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <>
                ...
                <>
                  <Save size={18} /> Submit All {tasks.length} Task
                  {tasks.length !== 1 ? "s" : ""}
                </>
              )}
            </button>
