...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="bg-white rounded-xl p-2 shadow-sm border border-gray-100 flex items-center gap-2 w-full min-w-0">
    ...
    <div className="min-w-0 flex-1">
      <p className="text-[9px] sm:text-[10px] text-gray-500 font-bold capitalize tracking-tight truncate mb-0.5 leading-tight">
        {label}
      </p>
      <p className="text-base sm:text-lg font-extrabold text-gray-900 truncate leading-none">
        {value}
      </p>
    </div>
  </div>
...
export default function MaintenanceView({
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      const hasSubmission =
        task.submission_date !== null && task.submission_date !== undefined;
  ...
  const processedData = useMemo(() => {
    ...
    currentTasks.forEach((task) => {
      ...
      const taskStartDate = task.originalTaskStartDate
        ? new Date(task.originalTaskStartDate)
        : null;
      const hasSubmission =
        task.submission_date !== null && task.submission_date !== undefined;
      const isTaskToday = taskStartDate ? isToday(taskStartDate) : false;
      const isOverdue = taskStartDate
        ? taskStartDate < today && !hasSubmission
        : false;
      ...
      // Monthly Cost
      const date = task.originalTaskStartDate
        ? new Date(task.originalTaskStartDate)
        : new Date();
    ...
    const freqData = Object.keys(freqCounts).map((key) => ({
      name: key,
      count: freqCounts[key],
    }));

    const costData = Object.keys(monthlyCost).map((key) => ({
      name: key,
      cost: monthlyCost[key],
    }));
    // Ensure some data for chart if empty
    if (costData.length === 0) {
      ["Jan", "Feb", "Mar", "Apr", "May"].forEach((m) =>
        costData.push({ name: m, cost: 0 }),
      );
    }
      ...
      <div className="space-y-4 pb-8 overflow-x-hidden w-full max-w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <StatCard
            ...
            value={processedData?.totalMachines || 0}
            color="bg-blue-500"
          />
          <StatCard
            icon={Calendar}
          ...
          <StatCard
            icon={IndianRupee}
            label="Total Cost"
            value={`₹${processedData?.totalCost || 0}`}
            color="bg-blue-500"
          />
        </div>
            ...
            <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 shadow-inner w-full sm:w-auto">
              {["today", "week", "month", "all"].map((f) => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 text-[10px] font-extrabold rounded-md transition-all uppercase tracking-tight text-center ${
                    maintFilter === f
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50 sticky top-0 z-20 shadow-sm backdrop-blur-sm">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider min-w-[200px]">
                    Task Description
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider min-w-[150px]">
                    Remarks
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">
                    Upload Image
                ...
                {filteredTasks.length > 0 ? (
                  filteredTasks.map((task, index) => {
                    const hasSubmission =
                      ...
                      task.submission_date !== undefined;
                    const taskDate = task.originalTaskStartDate
                      ? new Date(task.originalTaskStartDate)
                      : null;
                    const today = new Date();
                    today.setHours(0, 0, 0, 0);
                    const isOverdue =
                      taskDate && taskDate < today && !hasSubmission;
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                          {editingTaskId === task.id ? (
                            <input
                              ...
                              onChange={(e) =>
                                handleInputChange("part_area", e.target.value)
                              }
                              className="w-full px-2 py-1 border rounded text-xs"
                            />
                          ) : (
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">
                          {editingTaskId === task.id ? (
                            ...
                            new Date(task.originalTaskStartDate).toLocaleString(
                              "en-IN",
                              {
                                day: "2-digit",
                                month: "2-digit",
                        ...
                        <td className="px-4 py-3 text-sm text-gray-600">
                          {isAudioUrl(task.remarks) ? (
                            <AudioPlayer url={task.remarks} />
                          ) : (
                            task.remarks
                          )}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap text-center">
                          {editingTaskId === task.id ? (
                            ...
                            <button
                              onClick={() => {
                                setViewerMedia({
                                  url: task.uploaded_image_url,
                                  type: "image",
                                });
                                setViewerOpen(true);
                              }}
                              className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-medium text-xs hover:underline transition-colors"
                            >
