...
const StatCard = ({ icon: Icon, label, value, color }) => (
  <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex flex-col gap-1 relative overflow-hidden min-w-0">
    <div
      className={`absolute top-0 right-0 w-16 h-16 -mr-4 -mt-4 rounded-full ${color} opacity-5`}
    ></div>
    <div className="flex justify-between items-start z-10">
      <div className="min-w-0 flex-1">
        <p className="text-[10px] sm:text-[11px] text-gray-500 font-bold capitalize tracking-tight truncate mb-0.5">
          {label}
        </p>
        <p className="text-xl sm:text-2xl font-extrabold text-gray-900 truncate leading-none">
          {value}
        </p>
      </div>
  ...
  const processedData = useMemo(() => {
    if (!tasks || tasks.length === 0) {
      return {
        ...
        paymentData: [
          { name: "Full", value: 0, color: "#3B82F6" },
          { name: "Partial", value: 0, color: "#10B981" },
        ],
      };
    }
    ...
    const deptData = Object.keys(deptStats)
      .map((name) => ({
        ...
        rate: Math.round(
          (deptStats[name].completed / deptStats[name].total) * 100,
        ),
      }))
    ...
    const paymentData = [
      ...
      {
        name: "Partial",
        value: statusCounts.InProgress + statusCounts.Pending,
        color: "#10B981",
      },
    ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-lg font-bold text-gray-800 text-center w-full">
              Repair Status by Machine/Dept
            </h3>
          </div>
            ...
            {vendorData.length > 0 ? (
              vendorData.map((vendor, i) => (
                <div key={i} className="flex items-center gap-4">
                  <span
                    className="text-[11px] font-bold text-gray-500 w-24 truncate"
                    title={vendor.name}
                  >
                    {vendor.name}
                  </span>
                  <div className="flex-1 flex items-center gap-3">
                    <div className="flex-1 h-1.5 bg-orange-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(vendor.cost / maxVendorCost) * 100}%`,
                        }}
                      ></div>
                    </div>
                    <span className="text-[11px] font-bold text-gray-700 whitespace-nowrap min-w-[60px] text-right">
              ...
              <p className="text-center text-gray-400 text-xs w-full py-10">
