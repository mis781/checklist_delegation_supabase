...
export default function PendingPackaging({ data, filters, refresh }) {
  ...
  const renderCard = (order) => {
    ...
    const tatStatus = getTatStatusForOrder(order, "Packaging", tatRules, { isCompleted: false });
    ...
    const totalDispatchQty = pendingPackagingItems.reduce((sum, item) => sum + (parseFloat(item.dispatchQty) || 0), 0);
        ...
        <div className="border-t border-gray-100 pt-2">
          ...
          {isExpanded && (
            <div className="space-y-2 mt-2 pt-2 border-t border-dashed border-gray-200">
              {pendingPackagingItems.map((prod, idx) => (
                <div key={idx} className="bg-slate-50 p-2.5 rounded-lg border border-slate-200/70 text-[11px] space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-gray-600 text-[10px] pt-1">
                    ...
                    <div className="text-right">
                      ...
                      <span className="font-bold text-amber-600">Pending Packaging</span>
                    </div>
                  </div>
                </div>
              ))}
                        ...
                        pendingPackagingItems.map((item, idx) => {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                              ...
                              <td className="px-4 py-3 text-[11px] text-gray-700 text-center">{formatDate(item.dispatchDate)}</td>
