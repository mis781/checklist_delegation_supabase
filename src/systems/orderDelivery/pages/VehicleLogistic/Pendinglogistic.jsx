...
import { getPackagingHistory, getLogisticHistory, getDispatchHistory } from '../../utils/storageManager';
...
export default function Pendinglogistic({ data, filters, refresh }) {
  ...
  const tableHeaders = [
    ...
    { label: "Packing Image", className: "sticky right-[80px] bg-gray-50 z-20 shadow-[-1px_0_0_0_#e5e7eb] min-w-[90px]" },
  ...
  const renderCard = (order) => {
    ...
    const totalDispatchQty = pendingLogisticItems.reduce((sum, item) => {
      const qty = parseFloat(item.dispatchQty) ||
        parseFloat(allDispatch.find(d => d.dispatchId === item.dispatchId)?.dispatchQty) || 0;
      return sum + qty;
    }, 0);
    const packingImage = pendingLogisticItems.find(ph => ph.packagingImage)?.packagingImage || orderPackagedItems.find(ph => ph.packagingImage)?.packagingImage || null;
        ...
        <div className="border-t border-gray-100 pt-2">
          ...
          {isExpanded && (
            <div className="space-y-2 mt-2 pt-2 border-t border-dashed border-gray-200">
              {pendingLogisticItems.map((prod, idx) => (
                <div key={idx} className="bg-slate-50 p-2.5 rounded-lg border border-slate-200/70 text-[11px] space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-1 text-gray-600 text-[10px] pt-1">
                    ...
                    <div className="text-right">
                      ...
                      <span className="font-bold text-amber-600">Needs Vehicle</span>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Fragment key={order.orderId}>
        ...
        >
          ...
          <td className="px-3 py-3 whitespace-nowrap sticky right-[80px] z-10 shadow-[-1px_0_0_0_#e5e7eb] transition-colors bg-white group-hover:bg-slate-50 text-center" onClick={(e) => e.stopPropagation()}>
            {packingImage ? (
              <button onClick={(e) => handleImageView(packingImage, e)} className="text-indigo-600 hover:text-indigo-800 flex justify-center w-full focus:outline-none" title="View Packing Image">
                <Eye size={16} />
              </button>
            ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {pendingLogisticItems.map((item, idx) => {
                        ...
                        const dispatchQty = parseFloat(item.dispatchQty) || parseFloat(matchDispatch?.dispatchQty) || 0;
