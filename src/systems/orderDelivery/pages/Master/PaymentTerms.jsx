...
import { getPaymentTermsMaster, savePaymentTermsMaster } from '../../utils/storageManager';
import { generateId } from '../../utils/helpers';
import DataTable from '../../components/DataTable';
import ModalAlert from '../../components/ModalAlert';
import ModalForm from '../../components/ModalForm';

export default function PaymentTerms({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'PT-NO', 'Payment Term'];

  useEffect(() => {
    setData(getPaymentTermsMaster());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.ptNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleDelete = (id) => {
    showAlert('confirm', 'Delete Payment Term?', 'Are you sure you want to delete this Payment Term?', () => {
      const updated = data.filter(i => i.id !== id);
      savePaymentTermsMaster(updated);
      setData(updated);
      showAlert('success', 'Deleted!', 'The Payment Term has been removed successfully.');
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validEntries.length === 0) {
      showAlert('error', 'Error', 'Please provide at least one payment term.');
      return;
    }
    ...
    if (editingId) {
      ...
      savePaymentTermsMaster(updated);
      setData(updated);
      showAlert('success', 'Updated!', 'Payment Term has been updated.');
    } else {
      const newItems = validEntries.map((entry, index) => ({
        id: generateId() + index,
        name: entry.name,
        timestamp: new Date().toISOString(),
        ptNo: `PT-${String(data.length + index + 1).padStart(3, '0')}`
      }));

      const updatedData = [...data, ...newItems];
      savePaymentTermsMaster(updatedData);
      setData(updatedData);
      showAlert('success', 'Success!', `${newItems.length} term(s) added successfully.`);
    }
    setShowModal(false);
  };
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-[13px]">
      ...
      <td className="px-4 py-2 text-gray-600 whitespace-nowrap">{formatTimestamp(item.timestamp)}</td>
      <td className="px-4 py-2 text-gray-900 font-bold whitespace-nowrap">{item.ptNo}</td>
      <td className="px-4 py-2 text-gray-700 whitespace-nowrap uppercase font-medium">{item.name}</td>
    </tr>
  );
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.ptNo}</span><h3 className="text-[13px] font-medium text-gray-700 mt-1 uppercase">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Payment Term' : 'Register Payment Term'}
            ...
            {entries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  {index === 0 && <label className="block text-[10px] font-medium text-gray-700 uppercase tracking-tight">Payment Term *</label>}
                  <input
                    required
                    type="text"
                    value={entry.name}
                    ...
                    placeholder="e.g. 30 Days Credit, Net 30..."
                  />
                </div>
