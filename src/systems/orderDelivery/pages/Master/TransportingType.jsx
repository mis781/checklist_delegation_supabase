...
export default function TransportingType({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'TT-NO', 'Transporting Type'];

  useEffect(() => {
    setData(getTransportingTypes());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.ttNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleDelete = (id) => {
    showAlert('confirm', 'Delete Transporting Type?', 'Are you sure you want to delete this Transporting Type?', () => {
      const updated = data.filter(i => i.id !== id);
      saveTransportingTypes(updated);
      setData(updated);
      showAlert('success', 'Deleted!', 'The Transporting Type has been removed successfully.');
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validEntries.length === 0) {
      showAlert('error', 'Error', 'Please provide at least one transporting type.');
      return;
    }
    ...
    if (editingId) {
      ...
      saveTransportingTypes(updated);
      setData(updated);
      showAlert('success', 'Updated!', 'Transporting Type has been updated.');
    } else {
      const newItems = validEntries.map((entry, index) => ({
        id: generateId() + index,
        name: entry.name,
        timestamp: new Date().toISOString(),
        ttNo: `TT-${String(data.length + index + 1).padStart(3, '0')}`
      }));
      
      const updatedData = [...data, ...newItems];
      saveTransportingTypes(updatedData);
      setData(updatedData);
      showAlert('success', 'Success!', `${newItems.length} type(s) added successfully.`);
    }
    setShowModal(false);
  };
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-[13px]">
      ...
      <td className="px-4 py-2 text-gray-900 font-bold whitespace-nowrap">{item.ttNo}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.ttNo}</span><h3 className="text-[13px] font-medium text-gray-700 mt-1 uppercase">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Transporting Type' : 'Register Transporting Type'}
            ...
            {entries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  {index === 0 && <label className="block text-[10px] font-medium text-gray-700 uppercase tracking-tight">Transporting Type *</label>}
                  <input 
                    required 
                    type="text" 
                    value={entry.name} 
                    ...
                    placeholder="e.g. FOR, Ex Factory..." 
                  />
                </div>
